       ...
       if (wid<1) or (wid>Length(WindowList)) then
        ...
        begin
         if Length(WindowList)=0 then
          ...
          begin
           SetWindowPos(WindowList[wid-1].Handle,HWND_TOPMOST,0,0,0,0,SWP_NOMOVE or SWP_NOSIZE or SWP_NOACTIVATE);
           WriteLog('Окно "' + WindowList[wid-1].Name + '" теперь поверх всех.');
          end;
        end;
      end
     else WriteLog('Номер окна не введён.');
...
begin
 ...
 AddCommand('exit',ExitHandler,'Закрывает плаир.','Закрывает плаир.');
 AddCommand('close',CloseHandler,'Закрывает консоль.','Закрывает консоль.');
 AddCommand('restart',RestartHandler,'Перезапуск плаира.','Перезапуск плаира.');
 AddCommand('whvideo',WHvideoHandler,'Подгонка размеров видео по пропорциям.','Подгонка размеров видео по пропорциям.' + #13 + 'Использование: whvideo [Width] [Height] [DestinationWidth] [DestinationHeight]' + #13#13 + 'Два параметра ширины и высоты можно заменить константой:' + #13 + 'hd - 1280x720' + #13 + 'fullhd - 1920x1080' + #13 + '4k - 3840x2160' + #13 + '8k - 7680x4320' + #13 + '16k - 15360x8640' + #13#13 + 'Например команда "whvideo 500 600 fullhd" эквивалентна команде "whvideo 500 600 1920 1080"');
 AddCommand('time',TimeHandler,'Текущие дата и время.','Текущие дата и время.');
 AddCommand('systeminfo',SystemInfoHandler,'Информация о системе.','Информация о системе.');
 AddCommand('cmdtofile',CmdToFileHandler,'Выполнение команды и запись результата в файл.','Выполнение команды и запись результата в файл.' + #13 + 'Использование: cmdtofile [FilePath] [command] <attr1> <attr2>...');
 AddCommand('timer',TimerHandler,'Выключение по таймеру.','Выключение плаира по таймеру. (Или другое действие, выбранное в меню "После завершения воспроизведения")' + #13#13 + 'Использование 1: timer' + #13 + 'Проверка статуса таймера. Включен или нет, и если включен, сколько времени осталось.' + #13#13 + 'Использование 2: timer stop' + #13 + 'Остановка таймера.' + #13#13 + 'Использование 3: timer resume' + #13
 + 'Возобновление таймера.' + #13#13 + 'Использование 4: timer [time] <metrics> <time2> <metrics>...' + #13 + 'Установка таймера. time - время. По умолчанию задаётся в секундах, но это можно изменить указав необязательный параметр metrics. Он может принимать следущие значения: ' + #13 + ' s - секунды' + #13 + ' m - минуты' + #13 + ' h - часы' + #13 + ' d - сутки' + #13 + 'Можно указать несколько значений времени, тогда они суммируются. Например: timer 2 h 30 m' + #13 + 'Так же обратите внимание, что metrics необязательный параметр. То есть, можно написать и так: timer 3 h 40 43 64 21 m 40 m 43 59');
 AddCommand('hscrl',hscrlHandler,'Переключение горизонтальной прокрутки.','Переключение горизонтальной прокрутки.');
 AddCommand('clear',ClearHandler,'Очистка консоли.','Очистка консоли.');
 AddCommand('title',TitleHandler,'Задание заголовка окна консоли.','Задание заголовка окна консоли.');
 CommandsCount:=59;
 ...
 Commands[43].LongDesc:='Операции с окошками.' + #13#13 + 'Использование 1: window update' + #13 + 'Обновление списка окон. Необходимо использовать перед другими операциями.' + #13#13 + 'Использование 2: window list' + #13 + 'Вывод полного списка окон.' + #13#13 + 'Использование 3: window search [str]' + #13 + 'Поиск окна по заголовку.' + #13 + 'str - Строка поиска. Можно писать не полностью.' + #13#13 + 'Использование 4: window hide [wnum]' + #13 + 'Скрыть окно.' + #13 + 'wnum - Номер окна.' + #13#13 + 'Использование 5: window show [wnum]'
  + #13 + 'Показать окно.' + #13 + 'wnum - Номер окна.' + #13#13 + 'Использование 6: window rename [wnum] [name]' + #13 + 'Изменить заголовок окна.' + #13 + 'wnum - Номер окна.' + #13 + 'name - Новый заголовок.' + #13#13 + 'Использование 7: window alpha [wnum] [level]' + #13 + 'Установить уровень непрозрачности окна.'
  ...
  + #13#13 + 'Использование 10: window ghost [wnum]' + #13 + 'Окно висит поверх других и не реагирует на нажатия, пропуская их сквозь себя.' + #13 + 'wnum - Номер окна.'+ #13#13 + 'Использование 11: window topmost [wnum]' + #13 + 'Окно висит поверх других.' + #13 + 'wnum - Номер окна.';
 ...
 Commands[58].LongDesc:='Работа с каналами.'+#13#13+'Сам по себе плаир работает только с одним каналом и не воспроизводит несколько треков одновременно.'+#13+'С помощью этой команды вы можете заставить его делать это.'
   +#13#13+'Использование: channel [cmd] <args>'+#13+'cmd - Команда.'+#13+'args - Её аргументы.'+#13#13+'Список команд и аргументов:'+#13+'list - Список каналов.'+#13+' Аргументов нет. Показывает список существующих каналов.'+#13+' Канал под номером 0 - управляется плаиром. Существует всегда, удаление напрямую невозможно.'
   +#13+' * Можно удалить, если поменять местами с другим каналом.'+#13#13+'create - Создать канал. Аргументы:'+#13+' type - Тип канала. file - файл на компьютере, net - файл или поток в интернете.'+#13+' path - абсолютный путь к файлу или потоку.'+#13#13+'delete - Удалить поток. Аргументы:'+#13+' num - Номер потока. Можно уточнить командой channel list.'+#13#13+'switch - Поменять поток местами с нулевым. Аргументы:'+#13;
 ...
 if Hashed='C55B42784FD64C13CC298FBD6209423340F3EE238EEDAF2D78D04B1001EDC520824B0544F60993116EA2DB70492A242630D0410290D5F86317AC7FDF6011F56C' then // FluttershyIsBestPony
  begin
   {if AppLoaded then
    ...
    WriteLog('Разблокировка команд не работает из автозапуска.');}
   Exit;
  end;
